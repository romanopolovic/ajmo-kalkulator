...
import { Copy, MessageCircle, Check, Settings2, RefreshCw, Save } from 'lucide-react';
...
export default function ClientMessageModal({ open, onOpenChange }: { open: boolean, onOpenChange: (open: boolean) => void }) {
  ...
  const parseTemplate = (template: string) => {
    const servicesStr = totals.calculatedPhases.map(cp => cp.serviceName).join(' i ');
    
    // Extract base pricing strings from phases (e.g., "11 € / m³")
    const pricingDetails = totals.calculatedPhases.map(cp => {
      const baseDetail = cp.breakdown[0]?.detail;
      return baseDetail ? `${baseDetail} za ${cp.serviceName}` : '';
    }).filter(Boolean).join(', ');

    // Extract subservices
    const subs = totals.calculatedPhases.flatMap(cp => cp.breakdown.slice(1).map(b => b.name));
    const subsStr = subs.length > 0 ? `, uz dodatak za ${subs.join(', ').toLowerCase()}` : '';

    let res = template;
    res = res.replace(/\{\{ime_klijenta\}\}/g, estimate.clientName ? ` ${estimate.clientName}` : '');
    res = res.replace(/\{\{usluga\}\}/g, servicesStr || 'rada');
    res = res.replace(/\{\{kolicina\}\}/g, estimate.phases.map(p => p.quantity).join(' + ') || 'Po dogovoru');
    res = res.replace(/\{\{jedinica\}\}/g, estimate.phases.map(p => SERVICES[p.serviceId]?.unit || '').join(' ') || '');
    ...
    res = res.replace(/\{\{lokacija\}\}/g, estimate.location.address || 'Nije uneseno');
    res = res.replace(/\{\{google_maps\}\}/g, getMapsUrl() || 'Nije uneseno');
    res = res.replace(/\{\{cijena_jedinica\}\}/g, pricingDetails || 'prema dogovoru');
    res = res.replace(/\{\{dodatne_usluge\}\}/g, subsStr);
    
    res = res.replace(/\{\{putni_trosak\}\}/g, totals.travelTotal > 0 ? `\nPutni trošak: ${formatCurrency(totals.travelTotal)}` : '');
    res = res.replace(/\{\{napomena\}\}/g, estimate.clientNotes ? `\nNapomena: ${estimate.clientNotes}` : '');
  ...
  const generateMessage = (selectedStyle: MessageStyle) => {
    ...
    const servicesStr = totals.calculatedPhases.map(cp => cp.serviceName).join(' i ');
    const pricingDetails = totals.calculatedPhases.map(cp => cp.breakdown[0]?.detail ? `${cp.breakdown[0].detail} za ${cp.serviceName}` : '').filter(Boolean).join(', ');
    const subs = totals.calculatedPhases.flatMap(cp => cp.breakdown.slice(1).map(b => b.name));
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between mb-4 border-b border-border pb-4 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold uppercase tracking-tight text-white">Poruka Klijentu</h2>
            </div>
          </div>
          <Button variant="ghost" size="icon" onClick={() => setShowEditor(!showEditor)} className={showEditor ? 'text-yellow-500' : 'text-muted-foreground'}>
        ...
        {showEditor ? (
          <div className="flex-1 overflow-y-auto flex flex-col gap-4">
            <div className="bg-yellow-500/10 border border-yellow-500/20 p-3 rounded text-xs text-yellow-200">
              <span className="font-bold">Uređivač predloška</span> - Možete koristiti sljedeće varijable: 
              <code className="bg-black px-1 mx-1 rounded">{'{{ime_klijenta}}'}</code>
              <code className="bg-black px-1 mx-1 rounded">{'{{usluga}}'}</code>
              <code className="bg-black px-1 mx-1 rounded">{'{{ukupna_cijena}}'}</code>
              <code className="bg-black px-1 mx-1 rounded">{'{{broj_radnika}}'}</code>
              <code className="bg-black px-1 mx-1 rounded">{'{{cijena_jedinica}}'}</code>
              <code className="bg-black px-1 mx-1 rounded">{'{{dodatne_usluge}}'}</code>
              <code className="bg-black px-1 mx-1 rounded">{'{{trajanje}}'}</code>
              <code className="bg-black px-1 mx-1 rounded">{'{{datum}}'}</code>
              <code className="bg-black px-1 mx-1 rounded">{'{{lokacija}}'}</code>
              <code className="bg-black px-1 mx-1 rounded">{'{{google_maps}}'}</code>
            </div>
            <Textarea 
            ...
            />
            <Button onClick={saveCustomTemplate} className="w-full bg-yellow-500 text-black hover:bg-yellow-400">
          ...
          <div className="flex-1 overflow-y-auto flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3 shrink-0">
              <Button className="h-12 bg-green-600 hover:bg-green-500 text-white justify-center" onClick={handleWhatsApp}>
                <MessageCircle className="w-5 h-5 mr-2" />
                Otvori WhatsApp
              </Button>
              
              <Button variant="secondary" className="h-12 justify-center" onClick={handleCopy}>
