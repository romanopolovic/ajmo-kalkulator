...
import { JobEstimate, JobPhase, AdditionalExpense, CalculatedPhase } from '../types';
...
const generateId = () => `RS-${new Date().getFullYear()}${String(new Date().getMonth()+1).padStart(2, '0')}${String(new Date().getDate()).padStart(2, '0')}-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`;
  ...
  persist(
    (set, get) => ({
      ...
      updatePhase: (phaseId, updates) => set((state) => ({
        estimate: {
          ...state.estimate,
          phases: state.estimate.phases.map(p => p.id === phaseId ? { ...p, ...updates } : p)
        }
      })),
      ...
      toggleSubService: (phaseId, subServiceId) => set((state) => ({
        estimate: {
          ...state.estimate,
      ...
      updateEstimate: (updates) => set((state) => ({
      ...
      removeExpense: (expenseId) => set((state) => ({
        estimate: {
          ...state.estimate,
          additionalExpenses: state.estimate.additionalExpenses.filter(e => e.id !== expenseId)
        }
      })),
      
      reset: () => set({ estimate: { ...initialEstimate, id: generateId() } }),

      saveCurrentEstimate: () => set((state) => {
        const existingIndex = state.savedEstimates.findIndex(e => e.id === state.estimate.id);
...
export const calculateTotals = (estimate: JobEstimate) => {
  ...
  estimate.phases.forEach(phase => {
    ...
    let subTime = 0;
    const activeSubs = service.subServices?.filter(s => phase.selectedSubServices[s.id]) || [];
    ...
    const finalHours = phase.isManualHours && phase.manualHoursOverride !== undefined 
    ...
    if (service.pricingModel === 'TIERED' && service.tiers) {
      const tier = service.tiers.find(t => phase.quantity >= t.min && phase.quantity <= t.max) 
                   || service.tiers[service.tiers.length - 1];
      basePrice = tier.price * phase.quantity * phase.workers;
      breakdown.push({ name: `Osnovno (${phase.quantity} ${service.unit || ''} × ${phase.workers} rad.)`, amount: basePrice, detail: `${tier.price} € / ${service.unit || 'jed.'} po radniku` });
    } else if (service.pricingModel === 'PER_HOUR') {
      const totalBaseHours = phase.quantity * (service.timePerUnit || 1);
      basePrice = totalBaseHours * phase.workers * (service.basePricePerHour || 0);
      breakdown.push({ name: `Osnova (× ${phase.workers} rad.)`, amount: basePrice, detail: `${service.basePricePerHour} €/h po radniku` });
    } else if (service.pricingModel === 'FIXED') {
      basePrice = (service.fixedPrice || 0) * phase.workers;
      breakdown.push({ name: `Fiksna cijena (× ${phase.workers} rad.)`, amount: basePrice });
    }
    ...
    activeSubs.forEach(sub => {
      ...
      if (sub.type === 'PER_HOUR') {
        ...
        breakdown.push({ name: sub.name, amount: subPrice, detail: `+${sub.price} €/h po radniku` });
      } else if (sub.type === 'PER_UNIT') {
        subPrice = phase.quantity * sub.price * phase.workers;
        breakdown.push({ name: sub.name, amount: subPrice, detail: `${sub.price} € / ${service.unit || 'jed.'} po radniku` });
      } else if (sub.type === 'FIXED') {
        subPrice = sub.price * phase.workers;
        breakdown.push({ name: sub.name, amount: subPrice, detail: `Fiksno po radniku` });
      }
      phasePrice += subPrice;
    });

    // 5. Manual Price Override
    const finalPhasePrice = phase.isManualPrice && phase.manualPriceOverride !== undefined
      ? phase.manualPriceOverride
      : phasePrice;

    if (phase.isManualPrice && phase.manualPriceOverride !== undefined) {
      breakdown.push({ name: 'Ručna korekcija', amount: finalPhasePrice - phasePrice, detail: 'Administrator override' });
    }
  ...
  const travelTotal = (estimate.location.distanceKm || 0) * (estimate.location.travelCostPerKm || 0);
  const expensesTotal = estimate.additionalExpenses.reduce((sum, exp) => sum + exp.amount, 0);
  
  let subtotal = phasesTotal + travelTotal + expensesTotal;
  
  if (estimate.priority === 'urgent' || estimate.priority === 'same_day') subtotal *= 1.2;
