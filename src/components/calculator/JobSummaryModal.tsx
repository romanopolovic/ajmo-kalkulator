...
export default function JobSummaryModal({
  ...
  const getMapsUrl = (type: "google" | "apple") => {
    ...
    if (type === "apple")
      return `https://maps.apple.com/?q=${estimate.location.lat},${estimate.location.lng}`;
    return "";
  };
  ...
  const generateJobText = () => {
    ...
    if (totals.travelTotal > 0)
      text += `Putni trošak: ${formatCurrency(totals.travelTotal)}\n`;
    if (totals.expensesTotal > 0)
      text += `Dodatni troškovi: ${formatCurrency(totals.expensesTotal)}\n`;
    if (totals.discountAmount > 0)
      text += `Popust: -${formatCurrency(totals.discountAmount)}\n`;
  ...
  const handleShare = async () => {
    const shareUrl = `${window.location.origin}/share/${estimate.id}`;
    if (navigator.share) {
      try {
  ...
  const generateICS = () => {
    ...
    const startDate = formatICSDate(
      estimate.date || "",
      estimate.startTime || "09:00",
    );
    ...
    const serviceNames = totals.calculatedPhases
      .map((p) => p.serviceName)
      .join(" + ");
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            ...
            {estimate.location.lat && (
              <div className="flex gap-2 pt-2">
                <Button
                  ...
                  onClick={() => window.open(getMapsUrl("google"), "_blank")}
                >
                ...
                <Button
                  variant="outline"
                  className="flex-1 text-xs h-10"
                  onClick={() => window.open(getMapsUrl("apple"), "_blank")}
                >
            ...
            <div className="border border-border p-6 rounded-lg bg-black flex flex-col items-center justify-center mt-6">
              <span className="text-xs text-muted-foreground mb-4 uppercase tracking-wider font-bold">
                Klijent QR Link
              </span>
              <div className="bg-white p-2 rounded">
                <QRCodeSVG
                  value={`${window.location.origin}/share/${estimate.id}`}
                  size={120}
                  level="Q"
                  fgColor="#000000"
                  bgColor="#FFFFFF"
